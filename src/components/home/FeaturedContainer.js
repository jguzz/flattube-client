...
class FeaturedContainer extends Component {
  ...
  render() {
    return (<Container>
      {(this.state.isClicked) ? <ShowContainer user={this.props.user} video={this.state.currentVideo} users={this.props.users} /> : this.props.videos.map(video => <FeaturedVideoCard onClick={this.handleClick} key={video.id} video={video} />)}
    </Container>)
  }
}
