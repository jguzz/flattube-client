...
class FeaturedContainer extends Component {
  ...
  render() {
    return (<Container>
      {(this.state.isClicked) ? <ShowContainer handleLike={this.props.handleLike} video={this.state.currentVideo} /> : this.props.videos.map(video => <FeaturedVideoCard onClick={this.handleClick} key={video.id} video={video} />)}
    </Container>)
  }
}
